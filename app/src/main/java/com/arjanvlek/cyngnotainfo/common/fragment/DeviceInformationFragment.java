...
import com.arjanvlek.cyngnotainfo.common.model.DeviceInformationData;
...
import static com.arjanvlek.cyngnotainfo.common.internal.SystemVersionProperties.NOT_SET;
...
public class DeviceInformationFragment extends AbstractFragment {
        ...
        if(isAdded()) {
            ...
            if (devices != null) {
                for (Device device : devices) {
                    if (device.getModelNumber() != null && device.getModelNumber().equals(applicationData.CYANOGEN_DEVICE)) {
            ...
            if (!applicationData.CYANOGEN_VERSION.equals(NOT_SET)) {
                cyanogenOsVerView.setText(applicationData.CYANOGEN_VERSION);
            ...
            if (!applicationData.SECURITY_PATCH_DATE.equals(NOT_SET)) {
                osPatchDateView.setText(applicationData.SECURITY_PATCH_DATE);
            } else {
    ...
    private class GetDevices extends AsyncTask<Void, Void, List<Device>> {
        ...
        protected List<Device> doInBackground(Void... params) {
            try {
                return getApplicationData().getDevices();
            } catch (Exception e) {
                return null;
            }
        }
